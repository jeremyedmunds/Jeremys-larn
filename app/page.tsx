...
type Monster = { id: string; name: string; glyph: string; hp: number; maxHp: number; attack: number; xp: number; gold: number; sleep?:number; fear?:number; confused?:number; charmed?:boolean; haste?:number; behavior?:"smart"|"wander"|"ambush"; family?:"beast"|"humanoid"|"undead"|"demon"|"dragon"; invisible?:boolean; thief?:boolean; drain?:boolean; regen?:number; resist?:"fire"|"cold"|"magic"; ranged?:"fire"|"cold"|"magic"; guard?:boolean } & Point;
...
function monsterTraits(name:string){const n=name.toLowerCase();if(n.includes("wraith")||n.includes("vampire"))return {family:"undead" as const,behavior:"ambush" as const,invisible:n.includes("wraith"),drain:true,regen:n.includes("vampire")?2:1,resist:"cold" as const};if(n.includes("demon"))return {family:"demon" as const,behavior:"smart" as const,resist:"fire" as const,ranged:"magic" as const,guard:true};if(n.includes("dragon"))return {family:"dragon" as const,behavior:"smart" as const,resist:n.includes("white")?"cold" as const:"fire" as const,ranged:n.includes("white")?"cold" as const:"fire" as const,regen:1,guard:true};if(n.includes("gnome"))return {family:"humanoid" as const,behavior:"smart" as const,thief:!n.includes("king")};if(n.includes("orc")||n.includes("hobgoblin")||n.includes("troll"))return {family:"humanoid" as const,behavior:"smart" as const,regen:n.includes("troll")?2:0};return {family:"beast" as const,behavior:"wander" as const}}
...
function makeFloor(level: number, seed = Date.now()): Floor {
  ...
  for (let i = 0; i < 4 + level; i++) {
    ...
    monsters.push({ id: `m${level}-${i}-${seed}`, name: m[0], glyph: m[1], hp, maxHp: hp, attack: m[3], xp: m[4], gold: m[5], ...monsterTraits(m[0]), ...p });
  }
  const items: Item[] = [];
...
export default function Home() {
  ...
  const act = useCallback((dx: number, dy: number) => update(g => {
    ...
    if(g.monsterHold<=0)g.monsters.forEach(m => {Object.assign(m,{...monsterTraits(m.name),...m});if(m.regen&&m.hp<m.maxHp)m.hp=Math.min(m.maxHp,m.hp+m.regen);if(m.charmed)return;if((m.sleep??0)>0){m.sleep!--;return}const actions=(m.haste??0)>0?2:1;if((m.haste??0)>0)m.haste!--;for(let action=0;action<actions;action++){if((m.fear??0)>0){m.fear!--;const sx=Math.sign(m.x-g.player.x),sy=Math.sign(m.y-g.player.y),tx=m.x+sx,ty=m.y+sy;if(g.tiles[ty]?.[tx]!=="#"){m.x=tx;m.y=ty}continue}if((m.confused??0)>0){m.confused!--;const q=[[1,0],[-1,0],[0,1],[0,-1]][Math.floor(Math.random()*4)],tx=m.x+q[0],ty=m.y+q[1];if(g.tiles[ty]?.[tx]!=="#"){m.x=tx;m.y=ty}continue}
      const d=distance(m,g.player);if(d===1){const ward=(m.family==="undead"&&g.undeadProtection>0)||(["demon","dragon"].includes(m.family||"")&&g.spiritProtection>0)?4:0,dmg=Math.max(1,m.attack+Math.floor(Math.random()*3)-1-g.player.armor-ward);g.player.hp-=dmg;pushLog(g,`The ${m.name} hits you for ${dmg}.`);if(m.thief&&Math.random()<.28){const loot=g.bag.filter(i=>!i.active);if(loot.length){const stolen=loot[Math.floor(Math.random()*loot.length)];stolen.qty--;if(stolen.qty<=0)g.bag=g.bag.filter(i=>i.id!==stolen.id);pushLog(g,`The ${m.name} steals your ${stolen.name}!`)}else{const n=Math.min(g.player.gold,10+g.floor*5);g.player.gold-=n;m.gold+=n;pushLog(g,`The ${m.name} steals ${n} gold!`)}}if(m.drain&&Math.random()<.24){if(g.lifeProtection>0){g.lifeProtection=0;pushLog(g,"Life protection absorbs a level-draining touch.")}else{g.player.level=Math.max(1,g.player.level-1);g.player.xp=Math.max(0,g.player.xp-Math.floor(EXPERIENCE[Math.min(g.player.level,EXPERIENCE.length-1)]/3));g.player.maxHp=Math.max(5,g.player.maxHp-3);g.player.hp=Math.min(g.player.hp,g.player.maxHp);pushLog(g,`The ${m.name} drains one of your experience levels!`)}}continue}
      if(m.ranged&&d>=2&&d<=6&&Math.random()<.32){let dmg=3+Math.floor(g.floor/2)+Math.floor(Math.random()*5);if(m.ranged==="fire"&&g.fireResist>0)dmg=Math.ceil(dmg/3);g.player.hp-=dmg;pushLog(g,`The ${m.name} strikes from range with ${m.ranged} for ${dmg}.`);continue}
      if(g.stealth>0&&d>2)continue;if(m.guard&&distance(m,g.stairs)<4&&d>4)continue;let options:number[][];if(m.behavior==="wander"&&Math.random()<.7)options=[[1,0],[-1,0],[0,1],[0,-1]].sort(()=>Math.random()-.5);else if(m.behavior==="ambush"&&d>4)continue;else{const sx=Math.sign(g.player.x-m.x),sy=Math.sign(g.player.y-m.y);options=Math.random()>.5?[[sx,0],[0,sy]]:[[0,sy],[sx,0]]}for(const [mx,my] of options){const tx=m.x+mx,ty=m.y+my;if((mx||my)&&g.tiles[ty]?.[tx]!=="#"&&!(tx===g.player.x&&ty===g.player.y)&&!g.monsters.some(o=>o.id!==m.id&&o.x===tx&&o.y===ty)){m.x=tx;m.y=ty;break}}
    }});
  ...
  const consumeItem = useCallback((id:string)=>update(g=>{const item=g.bag.find(x=>x.id===id);if(!item||g.status!=="playing")return;const effect=item.effect;
    ...
    if(item.kind==="potion"){
      ...
      else if(effect==="haste monsters"){g.monsterHold=0;g.monsters.forEach(m=>m.haste=50);pushLog(g,"Every monster surges with unnatural speed.")}
  ...
  const restoreGame=useCallback((old:Game)=>{const base=freshGame();const floors=old.floors??base.floors;Object.entries(floors).forEach(([key,f])=>{f.features??=[];f.items??=[];f.monsters??=[];diversifyPotions(f.items,Number(key)*97);const bad=f.features.filter(x=>x.kind==="door"&&!x.used);bad.forEach(d=>f.tiles[d.y][d.x]=".");f.features=f.features.filter(x=>x.kind!=="door"||x.used);const candidates=doorwayCandidates(f.tiles).filter(p=>!f.items.some(i=>i.x===p.x&&i.y===p.y)&&!f.monsters.some(m=>m.x===p.x&&m.y===p.y));bad.slice(0,Math.min(3,candidates.length)).forEach((d,i)=>{const p=candidates[i];d.x=p.x;d.y=p.y;f.features.push(d);f.tiles[p.y][p.x]="+"})});const explored=old.explored??(old.floor===0?Array.from({length:H},()=>Array(W).fill(true)):blankExplored());revealAround(explored,old.player);const current=floors[old.floor],restored={...base,...old,tiles:current?.tiles??old.tiles,items:current?.items??old.items??[],monsters:current?.monsters??old.monsters??[],features:current?.features??old.features??[],deadline:old.deadline??2500,floors,explored,gems:old.gems??0,gemValue:old.gemValue??0,shopStock:old.shopStock??base.shopStock,player:{...base.player,...old.player,spells:old.player?.spells??base.player.spells}};safeBag(restored);setGame(restored)},[]);
  ...
  const projectileSpells=["mle","ssp","bal","cld","lit","mfi","fgr"];
  const fireSpell=useCallback((code:string,dx:number,dy:number)=>{setAiming(null);update(g=>{const s=SPELLS.find(x=>x[0]===code);if(!s)return;const tier=Math.floor(SPELLS.findIndex(x=>x[0]===code)/4)+1;if(g.player.mana<tier)return pushLog(g,"You lack the spell points.");g.player.mana-=tier;let x=g.player.x,y=g.player.y;for(let step=0;step<Math.max(W,H);step++){x+=dx;y+=dy;if(x<0||y<0||x>=W||y>=H||g.tiles[y][x]==="#"){pushLog(g,`${s[1]} strikes the stone and dissipates.`);break}const feature=g.features.find(f=>!f.used&&f.x===x&&f.y===y);if(feature?.kind==="mirror"){const reflected=Math.max(1,tier*5-Math.floor(g.player.armor/2));g.player.hp-=reflected;pushLog(g,`The mirror reflects ${s[1]} back at you for ${reflected} damage!`);break}if(feature?.kind==="door"||feature?.kind==="statue"){const power=tier*6+g.player.intelligence;if(power>=18){feature.used=true;g.tiles[y][x]=".";pushLog(g,`${s[1]} destroys the ${feature.kind}.`);if(feature.kind==="statue"&&Math.random()<.6){const choices=SPELLS.filter(q=>!g.player.spells.includes(q[0])),book=choices[Math.floor(Math.random()*choices.length)];if(book)g.items.push({id:`blast-book-${feature.id}`,name:`spellbook: ${book[1]}`,glyph:"+",kind:"spellbook",effect:book[0],amount:1,x,y})}}else pushLog(g,`${s[1]} strikes the ${feature.kind} without destroying it.`);break}const target=g.monsters.find(m=>m.x===x&&m.y===y);if(target){Object.assign(target,{...monsterTraits(target.name),...target});const element=code==="mfi"||code==="lit"?"fire":code==="cld"?"cold":"magic";let dmg=tier*6+g.player.intelligence;if(target.resist===element){dmg=Math.max(1,Math.ceil(dmg/4));pushLog(g,`The ${target.name} resists ${element} magic.`)}target.hp-=dmg;pushLog(g,`${s[1]} hits the ${target.name} for ${dmg}.`);if(target.hp<=0){g.player.xp+=target.xp;g.monsters=g.monsters.filter(m=>m.id!==target.id);if(target.gold)g.items.push({id:`spell-drop-${target.id}`,name:"monster gold",glyph:"$",kind:"gold",amount:target.gold,x:target.x,y:target.y});pushLog(g,`The ${target.name} dies${target.gold?` and drops ${target.gold} gold.`:"."}`)}break}if(step===Math.max(W,H)-1)pushLog(g,`${s[1]} vanishes into the darkness.`)}g.turn++})},[update]);
  const cast=useCallback((code:string)=>{if(projectileSpells.includes(code)){setAiming(code);setService("none");update(g=>pushLog(g,`Aim ${SPELLS.find(s=>s[0]===code)?.[1]} with a movement key. Escape cancels.`));return}update(g=>{const s=SPELLS.find(x=>x[0]===code);if(!s)return;const tier=Math.floor(SPELLS.findIndex(x=>x[0]===code)/4)+1;if(g.player.mana<tier)return pushLog(g,"You lack the spell points.");g.player.mana-=tier;
    if(code==="hel")g.player.hp=Math.min(g.player.maxHp,g.player.hp+12+g.player.wisdom);
    else if(["mle","ssp","bal","cld","lit","mfi","fgr"].includes(code)){const mirror=g.features.some(f=>f.kind==="mirror"&&!f.used);if(mirror&&Math.random()<.3){const reflected=Math.max(1,tier*4-Math.floor(g.player.armor/2));g.player.hp-=reflected;pushLog(g,`A mirror reflects ${s[1]} back for ${reflected} damage!`)}else{const target=[...g.monsters].sort((a,b)=>distance(a,g.player)-distance(b,g.player))[0];if(target){Object.assign(target,{...monsterTraits(target.name),...target});const element=code==="mfi"||code==="lit"?"fire":code==="cld"?"cold":"magic";let dmg=tier*6+g.player.intelligence;if(target.resist===element){dmg=Math.max(1,Math.ceil(dmg/4));pushLog(g,`The ${target.name} resists ${element} magic.`)}target.hp-=dmg;pushLog(g,`${s[1]} hits the ${target.name} for ${dmg}.`);if(target.hp<=0){g.player.xp+=target.xp;g.monsters=g.monsters.filter(m=>m.id!==target.id);if(target.gold)g.items.push({id:`spell-drop-${target.id}`,name:"monster gold",glyph:"$",kind:"gold",amount:target.gold,x:target.x,y:target.y});pushLog(g,`The ${target.name} dies${target.gold?` and drops ${target.gold} gold.`:"."}`)}}}}
    else if(["hld","stp","web"].includes(code)){g.monsterHold=20+tier*5;pushLog(g,"The monsters freeze in place.")}
    else if(code==="sle"){const targets=g.monsters.filter(m=>distance(m,g.player)<9&&!(["undead","demon"].includes((m.family??monsterTraits(m.name).family) as string)));targets.forEach(m=>m.sleep=18+tier*3);pushLog(g,`${targets.length} nearby monsters fall asleep; undead and demons resist.`)}
    ...
    else if(code==="str")g.player.strength+=2;else if(code==="dex")g.player.dexterity+=2;else if(code==="enl"){g.explored=Array.from({length:H},()=>Array(W).fill(true));g.awareness=0}else if(code==="cbl"){g.awareness=30;revealAround(g.explored,g.player)}else if(code==="chm"){const m=g.monsters.sort((a,b)=>distance(a,g.player)-distance(b,g.player))[0];if(m){const family=m.family??monsterTraits(m.name).family;if(family==="undead"||family==="demon")pushLog(g,`The ${m.name} is immune to charm.`);else{m.charmed=true;pushLog(g,`The ${m.name} is charmed and will no longer attack.`)}}}else if(code==="sca"){g.monsters.filter(m=>distance(m,g.player)<10).forEach(m=>m.fear=20);pushLog(g,"Nearby monsters flee in terror.")}else if(code==="cnf"){g.monsters.filter(m=>distance(m,g.player)<9).forEach(m=>m.confused=20);pushLog(g,"Nearby monsters stumble in confusion.")}
  ...
  const moveOrAim=useCallback((dx:number,dy:number)=>{if(aiming)fireSpell(aiming,dx,dy);else{setService("none");act(dx,dy)}},[aiming,fireSpell,act]);
  useEffect(() => { const onKey=(e:KeyboardEvent) => { if(e.key==="Escape"&&aiming){setAiming(null);update(g=>pushLog(g,"Spell aiming cancelled."));return}if (dirs[e.key]) { e.preventDefault();moveOrAim(dirs[e.key].x, dirs[e.key].y); } if(!aiming&&e.key.toLowerCase()==="q") drink();if(!aiming&&e.key.toLowerCase()==="c")setService("spells");if(!aiming&&e.key.toLowerCase()==="i")setService("inventory"); if(!aiming&&e.key.toLowerCase()==="e") escape(); }; window.addEventListener("keydown",onKey); return()=>window.removeEventListener("keydown",onKey); }, [aiming,moveOrAim,drink,escape,update]);

  const entities = useMemo(() => { const map=new Map<string,{glyph:string;kind:string;title:string}>(); game.items.forEach(i=>map.set(`${i.x},${i.y}`,{glyph:i.glyph,kind:`item ${i.kind}`,title:i.name})); game.monsters.forEach(m=>{const traits={...monsterTraits(m.name),...m};if(traits.invisible&&game.awareness<=0&&game.monsterDetection<=0)return;const states=[m.charmed?"charmed":"",(m.sleep??0)>0?"asleep":"",(m.fear??0)>0?"afraid":"",(m.haste??0)>0?"hasted":""].filter(Boolean).join(", ");map.set(`${m.x},${m.y}`,{glyph:m.glyph,kind:"monster",title:`${m.name} (${m.hp}/${m.maxHp})${states?` — ${states}`:""}`})}); map.set(`${game.player.x},${game.player.y}`,{glyph:"@",kind:"player",title:"You"}); return map; },[game]);
  ...
  return <main className="game-shell">
    ...
    <section className="layout">
      ...
      <section className="map-wrap">
        {aiming&&<div className="aiming-banner">AIMING: {SPELLS.find(s=>s[0]===aiming)?.[1]} — press a direction · Esc cancels</div>}
        ...
        <div className="mobile-controls"><button onClick={()=>moveOrAim(0,-1)}>↑</button><div><button onClick={()=>moveOrAim(-1,0)}>←</button><button onClick={()=>moveOrAim(0,1)}>↓</button><button onClick={()=>moveOrAim(1,0)}>→</button></div></div>
      </section>
    ...
    <footer><div><kbd>WASD</kbd><kbd>ARROWS</kbd><span>{aiming?"AIM SPELL":"MOVE / ATTACK"}</span></div><div><kbd>I</kbd><span>INVENTORY</span></div><div><kbd>C</kbd><span>SPELLS</span></div><div><kbd>Q</kbd><span>DRINK POTION</span></div><div className="turns"><span>TIME REMAINING</span><strong>{Math.max(0,game.deadline-game.turn)}</strong></div></footer>
  </main>;
}
